...
def cross_entropy_with_probs(probs, targets, epsilon=1e-20):
    ...
    # loss = -(torch.log(probs + epsilon).gather(-1, targets.long().unsqueeze(-1).unsqueeze(-1))).squeeze(-1)
    loss = -(torch.log(probs + epsilon).gather(-1, targets.long().unsqueeze(-1))).squeeze(-1)
